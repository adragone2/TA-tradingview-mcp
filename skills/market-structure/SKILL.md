...
- **`legs_classify`** — each leg between swings as an impulse or a pullback, from three measurements (body share, colour agreement, closes near the extreme), plus whether a pullback is simple or complex. It is the unit the trend is built from, and it flags a "pullback" larger than the impulse before it — which is a trend change, not a retracement.
  ...
  **Also read `time_corrections`.** Corrections come in two kinds and leg classification only sees one. A PRICE correction retraces and has a depth; a TIME correction digests horizontally at low volatility and produces no leg with any depth at all — so a depth-based reading reports "no pullback" and skips a setup that is digesting perfectly well. It is also what a broken trendline actually predicts: Shannon says a trendline break "typically signals only that the rate of change has slowed, and that the stock is likely to experience a correction through time."
  Treat it as a **volatility state, never a direction** — it fires on 88% of random walks and 91.7% of real symbols, so firing distinguishes almost nothing. Whether it resolves with the prior trend is **not settled**: +8.3 points on daily (n=18) against −2.8 at 60-minute (n=14), neither significant.
- **`stage_plan`** — the longer timeframe as a gate (Stage 2 or 4, or no setup) and the shorter one as the action: ANTICIPATE / PARTICIPATE / EXIT / AVOID. It abstains when its clauses disagree, which is what `wyckoff_phase` cannot do. But it was **forward-tested and does not improve outcomes** (long 33.5% vs a 36.4% baseline over 198 independent events), so read it as a description of alignment and never as evidence for the trade.
