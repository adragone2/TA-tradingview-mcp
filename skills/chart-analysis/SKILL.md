...
- **Divergence: quote the AGREEMENT count, never a lone divergence.** One divergence appears on **99%** of random walks, seven times per walk. Two or more indicators agreeing appears on **13.5%**. The filter is the entire signal.
- **A Wyckoff phase is descriptive, not evidence.** `classifyPhase` returned a confident phase on **100%** of random walks — it never abstains. Springs and upthrusts fired on **0%**, and those are worth acting on.
- **`volatility_state` is context, not a setup.** The multi-bar coils (2BNR/3BNR/4BNR/8BNR) catch a market compressing across a week that NR4 and NR7 miss. But every pattern in it fires on **100%** of random walks, and the contraction/expansion principle behind it has **no lift over noise** — 76.4% on real data against 80.2% on a random walk. Say "this is coiled, expect a wider bar" and take direction from structure.
- **Structural patterns: read `noise_check` FIRST.** The noise floor is now 0.73 patterns per 200-bar random walk (unified 200-walk harness; the previously quoted 0.78 was a 40-walk run), down from 19.3, so a double top is worth reading. But the `rectangle` family still appears at ~0.17 per walk, channels on 33.5% (only 12% stable across three windows), and the wedges in 8-18% — treat those four with suspicion, especially in a choppy regime.
...
- **`patterns_lmw` is a second opinion, not a screen.** The textbook Lo/Mamaysky/Wang definitions match **37.9% of five-pivot windows from pure random walks** (43.4% before the P2.7 kernel ordering fix) — on live AAPL bars they returned 36 detections where our detector returned 2. Read it only where it *disagrees* with `patterns_detect`, and never quote its count.
