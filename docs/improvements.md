...
| P0.5 | **`ta_walls.js` has the same spread trap** — `applyWallsForSymbols` (~line 405) calls `chart.setSymbol` on a bare ticker with no `resolveTaSymbol` and no post-load identity check. Latent because the walls list is ETFs in practice. Route it through the resolver + verify the loaded series. (Found during P0.2's review.) | todo | Resolver wired; identity check on load |
